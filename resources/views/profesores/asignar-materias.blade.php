...
<div class="container py-5">
    <div class="card shadow-lg border-0 rounded-4">
        <div class="card-header bg-dark text-white text-center fw-bold fs-4">
            <i class="fas fa-chalkboard-teacher"></i> Asignar Materias a {{ $profesor->nombre }} {{ $profesor->apellido }}
        </div>
            ...
            <form action="{{ route('profesores.store-materias', $profesor->id) }}" method="POST"
                ...
                <div class="row g-4">
                    ...
                    <div class="col-md-4">
                        ...
                        <select id="unidad_curricular" name="unidad_curricular" class="form-select border-danger shadow-sm" required>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function validarCampos() {
        if (carrera.value && grado.value && unidadCurricular.value) {
    ...
    // Depura los valores antes de hacer la solicitud
    console.log('Carrera ID:', carreraId, 'Grado ID:', gradoId);
    ...
    if (carreraId && gradoId) {
        fetch(`/unidades-curriculares/${carreraId}/${gradoId}`)
            ...
            .catch(error => console.error('Error al obtener unidades curriculares:', error));
    }
}
    ...
    carrera.addEventListener("change", cargarUnidadesCurriculares);
